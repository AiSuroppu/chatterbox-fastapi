...
class WeightedWindow(BaseModel):
    ...
    size: int = Field(4, ge=3, le=15, description="The number of words in the sliding window.")
    average_score_threshold: float = Field(0.6, ge=0.0, le=1.0, description="Fail if the weighted average score of the words in the window falls below this threshold.")
...
class GapHallucinationCheck(BaseModel):
    ...
    min_gap_duration_ms: int = Field(800, ge=0, description="Do not check gaps shorter than this duration (ms). Prevents false positives in tight word spacing.")
    max_speech_in_gap_ms: int = Field(500, ge=0, description="Fail validation if more than this duration (ms) of VAD-detected speech is found in a single gap.")
...
class AlignmentValidationOptions(BaseModel):
    ...
    consecutive_poor_limit: Optional[int] = Field(
        2, ge=2, description="Fail if N consecutive words have a 'Poor' (2) or 'Critical' (3) severity. Set to null to disable.")
    consecutive_weak_limit: Optional[int] = Field(
        3, ge=2, description="Fail if N consecutive words have a 'Weak' (1) or higher severity. Set to null to disable.")
